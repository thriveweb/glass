...
exports.onCreateNode = ({ node, actions, getNode }) => {
  ...
  if (node.internal.type === 'MarkdownRemark') {
    ...
    if(slug.includes('model-types')) {
      slug = slug.replace('model-types', 'models')
    }

    if(slug.includes('post')) {
      slug = slug.replace('post', 'blog-post')
    }

    if(slug.includes('/info-page')) {
      slug = slug.replace('/info-page', '')
    }

    if(slug.includes('fa-qs')) {
      slug = slug.replace('fa-qs', 'faqs')
    }
